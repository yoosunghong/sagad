...
- [x] Build an automation script to bake the shared deformation masks into Static Mesh Vertex Colors (RGBA), **seam-coherent**: coincident UV-seam duplicates must receive identical RGBA (merge-then-average) or WPO tears the seam (ARCHITECT §2.4). *(`scripts/bake_vertex_colors.py`: policy/gains mask → A=mobility bake convention → seam merge-then-average → GLB COLOR_0 + sidecar JSON. Verified on gray-big-rock: 135 seam groups, raw policy shifted RGBA up to 0.35 across coincident duplicates before merge.)*
- [x] Author custom Unreal Engine 5 Materials utilizing World Position Offset (WPO) driven by Vertex Color channels, reading the **per-instance variant vector from Per-Instance Custom Data** (bend/noise/scale gains, floor count). *(`Plugins/sagad/Content/Python/build_wpo_material.py` builds `M_SagadWPO` in-editor: WPO = (bend+noise+scale)·A, vertex-color masks × PICD gains, strength ScalarParameters. In-editor run/visual validation pending.)*
- [x] Build a placement/scatter tool that calls `diffusion.sample(K)` → `spec.from_model` → writes K variant vectors as Per-Instance Custom Data on K HISM/Nanite instances. *(`ASagadScatterActor`: `SampleVariants` bridge → `HISM.NumCustomDataFloats` → per-instance `AddInstance` + `SetCustomDataValue`; CallInEditor `Scatter`/`ClearInstances`. Plugin compiles. In-editor scatter run pending.)*
